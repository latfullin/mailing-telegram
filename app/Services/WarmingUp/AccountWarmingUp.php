...
class AccountWarmingUp
{
  ...
  public function warmingUpAccount()
  {
    foreach ($this->phones as $phone) {
      $this->collections[$phone]["self"] = Telegram::instance(
        $phone
      )->getSelf();
      $this->collections[$phone]["dialogs"] = Telegram::instance(
        $phone
      )->getDialogs();
  ...
  private function setInformationProfile(string $phone): void
  {
    ...
    if (!$mePhoto) {
      ...
      Telegram::instance($phone)->updateNameProfile(
        $name,
        about: "Меня зовут {$name}. Живу в Спб :)"
      );
      Telegram::instance($phone)->updatePhotoProfile(
        $this->collections["photo"][
          rand(0, count($this->collections["photo"]) - 1)
        ]
      );
      Telegram::instance($phone)->sendMessage(
        "@hitThat",
        "{$name} - set new informations"
      );
    }
  }
  ...
  private function getIdChannel(array $array): void
  {
    ...
    foreach ($channelsId["chats"] as $channel) {
      if ($channel["_"] === "channel") {
        $this->collections["channels"][$channel["id"]] = [
          "id" => $channel["id"],
  ...
  private function joinChannelForWarmingUp($phone): bool
  {
    if ($this->collections[$phone] ?? false) {
      foreach ($this->collections["channels"] as $channel) {
        array_walk($this->collections[$phone]["dialogs"], function ($i) use (
          $channel,
          $phone
        ) {
          if ($i["_"] == "peerChat") {
            if ($i["chat_id"] !== $channel["id"]) {
              Telegram::instance($phone)->joinchannel(
